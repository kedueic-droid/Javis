...
指揮中心是共用透視相機的 **3D 物件空間**：J.A.R.V.I.S. 標題、模組欄、卡片、狀態列、全息環與內嵌舞台位於不同深度。拖曳任一 HUD 物件時，其餘物件會以相對位移與遠近縮放跟著動。

- 拖曳空白 HUD、卡片外框，或頂部 **「操控場景」** 手柄：旋轉場景（yaw / pitch）
- `Shift` 或滑鼠中鍵拖曳：平移
- 觸控板雙指開合、`Ctrl`（Windows）／`⌘`（macOS）+ 滾輪：縮放深度
- 單指拖曳亦可旋轉；雙指開合同時可縮放
- 雙擊「操控場景」，或在手柄聚焦時按 `0`／`Home`：重設視角
- 手柄聚焦時方向鍵微調角度，`+`／`-` 縮放
- **內嵌 iframe** 與 **分隔線** 仍可獨立操作，不會被場景手勢搶走
- `prefers-reduced-motion` 會關閉慣性與自動動畫，但仍保留靜態深度層次
