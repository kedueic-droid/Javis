...
這是一個不需後端的 SPA：Vite + React + TypeScript + Tailwind CSS + Three.js。五個既有應用以 **太陽系行星** 接入指揮中心，不會改寫那些專案的原始碼。

主畫面是立體星系：中央發光球體是 **J.A.R.V.I.S. 核心（太陽）**，每個應用是繞行的行星。點選行星預設在同一頁內嵌開啟，不會自動另開分頁。
...
點行星（或側欄模組）預設在 **指揮中心同頁內嵌舞台** 開啟，相機會輕輕對準該行星；可用拖曳把手調整星系區 / 內嵌區 / 側欄大小（尺寸會記在 localStorage）。舞台上的「外部開啟」才會另開分頁。若對方站台設了 `X-Frame-Options`，iframe 可能空白，請用「外部開啟」，系統不會自動跳新分頁。
...
2. 填名稱、說明、網址、技術棧與圖示。
3. 「加入啟動器」後立即以新行星出現在星系中，並寫入此瀏覽器的 `localStorage`。
...
主舞台是 WebGL **太陽系**：J.A.R.V.I.S. 為中央太陽，各應用為不同軌道半徑、速度與傾角的行星球體。拖曳空白處可環繞整個星系；拖曳行星可輕拉它離開軌道，鬆手後會彈回。

**如何開啟應用**

- 點選／點擊行星：選取並在同頁投影（內嵌 iframe）
- 寬螢幕左側 **MODULES** 清單、指令監視台（`/`）亦可啟動
- 未設定網址或已停用的行星會改開「設定」
- 點太陽、雙擊星系空白處，或「操控場景」雙擊／`0`／`Home`：回到總覽視角

**相機**

- 拖曳星系空白處，或頂部 **「操控場景」** 手柄：環繞旋轉
- `Shift` 或滑鼠中鍵拖曳手柄：平移
- 滾輪、觸控板雙指開合、`Ctrl`（Windows）／`⌘`（macOS）+ 滾輪：縮放
- 單指拖曳亦可旋轉；雙指開合同時可縮放
- 手柄聚焦時方向鍵微調角度，`+`／`-` 縮放
- **內嵌 iframe** 與 **分隔線** 仍可獨立操作，不會被場景手勢搶走
- `prefers-reduced-motion` 會暫停（或接近靜止）行星公轉，但仍保留球體、軌道與深度；相機操作仍可用
...
- `prefers-reduced-motion` 會關閉掃描線、開機動畫，並暫停行星公轉，但仍保留立體球體與軌道。
- 手機可單指旋轉星系、點選行星啟動；寬螢幕另有左欄模組清單。內嵌時改為可拖曳分割版面，星系留在一側。
